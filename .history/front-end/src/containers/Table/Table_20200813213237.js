  ...
  const onClickHandler = event => {
    ...
    if (+id === 3) {
      ...
      if (sortLabel.sortByCourse) {
        filterValute(
          state.valute.sort((prev, next) => {
            console.log(parseFloat(next.value._text) - parseFloat(prev.value._text))
            return parseFloat(next.value._text) - parseFloat(prev.value._text)
          })
        );
      } else {
        filterValute(
          state.valute.sort((prev, next) => prev.value._text - next.value._text)
        );
      }
