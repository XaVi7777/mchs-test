...
module.exports = function requestCourse(newDate) {
  const URL = date ? `http://www.cbr.ru/scripts/XML_daily.asp?date_req=${newDate}` : 'http://www.cbr.ru/scripts/XML_daily.asp';
  ...
  request({
    ...
    if (!error && response.statusCode === 200) {
      ...
      if (newDate) {
        const course = new CurrentCourse.findOneAndUpdate(options) || new CurrentCourse(options);
        await course.save();
      }
